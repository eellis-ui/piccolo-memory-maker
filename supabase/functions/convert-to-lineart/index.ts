    ...
    if (marker === 0xE1) { // APP1 (EXIF)
      ...
      if (exif[0] === 0x45 && exif[1] === 0x78 && exif[2] === 0x69 && exif[3] === 0x66) {
        ...
        for (let i = 0; i < entries; i++) {
          ...
          if (read16(entryOffset) === 0x0112) return read16(entryOffset + 8);
        }
      }
      break;
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Shared-photos bundle: copy a sibling's converted file if available.
    ...
    // STEP 1: Resize to max 1536px and detect actual orientation
    ...
    try {
      ...
      console.log(`Input: ${width}x${height} exif=${exifOri} => ${actualIsLandscape ? "LANDSCAPE" : "PORTRAIT"}`);
      const MAX_DIM = 1536;
    ...
    // STEP 2: OpenAI gpt-image-1 image edit. This produces clean published-style
    // line art directly — far more consistent than the previous Gemini pipeline,
    // which needed heavy post-processing to get an OK result. The trade-off:
    // OpenAI is slower (60-90s) and more expensive (~$0.17/image at high quality)
    // but the output is reliably good enough to ship to a paying customer.
...
OUTPUT: ${actualIsLandscape ? "LANDSCAPE orientation (wider than tall)" : "PORTRAIT orientation (taller than wide)"}. Fill the entire frame edge-to-edge with no margins. Same composition as the input photo.`;
    ...
    try {
      ...
      if (openaiResp.ok) {
        const r = await openaiResp.json();
        if (r.data?.[0]?.b64_json) imageBase64 = r.data[0].b64_json;
      } else {
        const errText = await openaiResp.text();
        console.error("OpenAI failed:", openaiResp.status, errText);
        lastError = `OpenAI error ${openaiResp.status}`;
      }
    } catch (e) {
    ...
    if (!imageBase64) {
      ...
      return new Response(JSON.stringify({ error: lastError || "Conversion failed." }), {
        status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    // STEP 3: Light cleanup — threshold to pure B&W (kills any anti-aliased grey)
    // and resize to A4. OpenAI line art is already mostly black/white so we don't
    // need closing, despeckle, fill removal, etc.
    ...
    try {
      ...
      // B&W threshold @ 200 — lenient to preserve all the line detail OpenAI drew.
      ...
      await supabase.from("order_photos").update({ conversion_status: "failed" }).eq("id", photoId);
      return new Response(JSON.stringify({ error: `Post-processing failed: ${ppErr}` }), {
        status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    // STEP 4: Upload and finalise
    ...
    // Mirror the converted file across the bundle's sibling rows.
    if (!photo.orders.unique_photos && (photo.orders.bundle_id || photo.orders.builder_session_id)) {
      ...
      if (sibIds.length > 0) {
        await supabase.from("order_photos").update({
          converted_path: convertedPath,
          conversion_status: "completed",
        }).in("order_id", sibIds).eq("page_position", photo.page_position);
      }
    }
